  ...
  <div class="content-wrapper">
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">View Interviewee Deatails</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
              ...
              <div class="card-body">
               ...
               if(isset($_POST['select_question_data'])){
                         $role_id_of_admin=$_POST['role'];           
                          $mysql_select_user_data = "SELECT * FROM user_login WHERE role='$role_id_of_admin' ORDER BY u_id DESC";
                          $result_select_user_data = mysqli_query($conn, $mysql_select_user_data);
                           $count_of_interviewee=mysqli_num_rows($result_select_user_data);
                           if($count_of_interviewee!=0){              
 ?> 
                  ...
                  $i=0;
                    ...
                    <td><?php $final_status=$row_select_user_data['final_status']; 
                     if($final_status==0){
                      ...
                      ?>
                      <button type="button" class="btn btn-success">Shortlisted</button>
                      <?php
                    }else{
                     ?>
                    ...
                    if($user_status==0){
                      ...
                      <?php }?>
                       <?php
                    if($user_status==0){
                      ?><a href="view_user_by_role.php?user_delete_id=<?php echo $row_select_user_data['u_id'];?>"><button type="button" class="btn btn-danger">Delete</button></a><?php }?>
                      <?php
                      if($user_status!=0){
                      ?>
                      <a href="view_user_all_details.php?u_id=<?php echo $row_select_user_data['u_id'];?>"><button type="button" class="btn btn-dark">View</button></a>
                      <?php
                    }else{
...
if(isset($_GET['user_delete_id'])){
  ...
  if(mysqli_query($conn,$mysql_detele_user)){
    header("Location: view_user_by_role.php");
  }
}
?>